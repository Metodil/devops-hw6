...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "node1" do |node1|
    ...
    node1.vm.provision "shell", path: "install_docker_centos.sh"
    node1.vm.provision "shell", path: "add_docker_metrics.sh"
    node1.vm.provision "shell", path: "install_prometheus.sh"
  ...
  config.vm.define "node2" do |node2|
    ...
    node2.vm.provision "shell", path: "install_goprom_node2.sh"
  end
  ...
  config.vm.define "node3" do |node3|
    ...
    node3.vm.provision "shell", path: "install_goprom_node3.sh"
  end
